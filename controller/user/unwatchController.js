...
router.post("/unwatch", (req, res) => {
    ...
    fs.readFile(maindatabase, "utf8", (err, data) => {
        if (err) {
            ...
            const user = maindata.find((u) => u.Session.includes(session));
            //jika tidak ditemukan
            if (!user) {
                return res.status(200).json({ code: 200, message: "Unwatch, but session expired" });
            }
            ...
            if (user) {
                ...
                //find in index array
                const index = user.Session.indexOf(session);
                

                //delete session and expired by index
                if (index > -1) {
                    user.Session.splice(index, 1);
                    user.Expired_Session.splice(index, 1);
                }
                ...
                fs.writeFile(maindatabase, JSON.stringify(maindata, null, 2), (err) => {
                    if (err) {
                        ...
                        console.log(chalk.cyan(dateformat()), `User ${user.Name} Unwatch`);
                        res.status(200).json({ code: 200, message: "UnWatch success" });
                    }
                });
            } else {
